...
def swot_top5(d):
    """
    Returns:
      weaknesses – list of (name, sev_label, sev_class)    top 5 from fears()
      strengths  – list of (name, score, category_colour)  top 5 from greeds()
    """
    weak   = [(nm, lbl, cls) for nm, lbl, cls, _ln in fears(d)][:5]
    G      = greeds(d)
    flat   = []
    for cat, items in G.items():
        for s, nm, _ln in items:
            flat.append((s, nm, GREED_COLS[cat]))
    flat.sort(key=lambda x: x[0], reverse=True)
    strong = [(nm, s, col) for s, nm, col in flat[:5]]
    return weak, strong


def swot_row_w(nm, lbl, cls):
    col = SEV_COLOR.get(cls, "#f59e0b")
    return (
        f"<div style='display:flex;align-items:center;gap:8px;padding:7px 2px;"
        f"border-bottom:1px solid #232d3a'>"
        f"<span style='color:{col};font-size:15px;line-height:1'>✕</span>"
        f"<span style='color:#e2e8f0;font-size:13px;flex:1'>{nm}</span>"
        f"<span style='background:{col};color:#0f1923;font-size:9px;font-weight:700;"
        f"padding:1px 7px;border-radius:8px;white-space:nowrap'>{lbl}</span></div>"
    )


def swot_row_s(nm, s, cat_col):
    if s >= 4.5:   glbl, gcol = "Very High", "#16a34a"
    elif s >= 3.5: glbl, gcol = "High",      "#22c55e"
    else:          glbl, gcol = "Moderate",  "#4ade80"
    return (
        f"<div style='display:flex;align-items:center;gap:8px;padding:7px 2px;"
        f"border-bottom:1px solid #232d3a'>"
        f"<span style='color:{cat_col};font-size:15px;line-height:1'>✓</span>"
        f"<span style='color:#e2e8f0;font-size:13px;flex:1'>{nm}</span>"
        f"<span style='background:{gcol};color:#021c0a;font-size:9px;font-weight:700;"
        f"padding:1px 7px;border-radius:8px;white-space:nowrap'>{glbl}</span></div>"
    )
...
for name in picked:
    ...
    # ─── 7. TOP-5 STRENGTHS & WEAKNESSES ────────────────────────────────
    st.markdown("---")
    st.markdown("#### 🔎 Country Assessment — Top 5 Strengths & Weaknesses")
    weak, strong = swot_top5(d)
    sw_col, st_col = st.columns(2)

    with sw_col:
        st.markdown(
            "<div style='color:#ef4444;font-size:10px;font-weight:700;letter-spacing:1.5px;"
            "text-transform:uppercase;margin-bottom:4px'>⬇ Top 5 Weaknesses</div>",
            unsafe_allow_html=True)
        if weak:
            body = "".join(swot_row_w(nm, lbl, cls) for nm, lbl, cls in weak)
            st.markdown(
                f"<div style='background:#1b222b;border-radius:10px;padding:10px 14px'>{body}</div>",
                unsafe_allow_html=True)
        else:
            st.write("_No significant weaknesses flagged._")

    with st_col:
        st.markdown(
            "<div style='color:#22c55e;font-size:10px;font-weight:700;letter-spacing:1.5px;"
            "text-transform:uppercase;margin-bottom:4px'>⬆ Top 5 Strengths</div>",
            unsafe_allow_html=True)
        if strong:
            body = "".join(swot_row_s(nm, s, col) for nm, s, col in strong)
            st.markdown(
                f"<div style='background:#1b222b;border-radius:10px;padding:10px 14px'>{body}</div>",
                unsafe_allow_html=True)
        else:
            st.write("_No significant strengths flagged._")

    # ─── 8. MARKET CONTEXT EXPANDER ─────────────────────────────────────
